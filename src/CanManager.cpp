...
//                 {
//                     fun.appendToCSV_DATA(file_name, (float)motor->nodeId, tMotor->initialJointAngle, INIT_SIGN);
//                 }
//                 else if (std::shared_ptr<MaxonMotor> maxonMotor = std::dynamic_pointer_cast<MaxonMotor>(motor_pair.second))
//                 {
//                     fun.appendToCSV_DATA(file_name, (float)motor->nodeId, maxonMotor->initialJointAngle, INIT_SIGN);
//                 }
//             }
...
// void CanManager::appendToCSV_State(const std::string& filename, string state, string sub_state) {
//     auto now = std::chrono::high_resolution_clock::now();
//     std::chrono::duration<float> elapsed = now - start;
//     std::ofstream file;
//     std::string fullPath = basePath + filename + ".txt";  // 기본 경로와 파일 이름을 결합
