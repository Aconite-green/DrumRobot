      ...
      sensor(sensorRef)
{
    ...
    start_DR = chrono::system_clock::now();
}
...
const std::string basePath_DR = "../../READ/";  // 기본 경로

// 변수를 CSV 파일에 한 줄씩 저장하는 함수
void DrumRobot::appendToCSV_DR(const std::string& filename) {
    auto now = std::chrono::high_resolution_clock::now();
    std::chrono::duration<float> elapsed = now - start_DR;
    std::ofstream file;
    std::string fullPath = basePath_DR + filename;  // 기본 경로와 파일 이름을 결합
