...
int main()
{
    ...
    send_frame_and_receive_reply(can_socket, &can_frame, motorlimits);
    ...
    send_frame_and_receive_reply(can_socket, &can_frame, motorlimits);
    printf("motor entercontrol mode?\n");
    usleep(50000);
    ...
    // Exit motor
    pack_cmd(motorlimits, &can_frame, command.motor_id, 0, 0, 0, 0, 0);
    send_frame_and_receive_reply(can_socket, &can_frame, motorlimits);
    usleep(10000);
    exitControlmode(&can_frame, command.motor_id);
    send_frame_and_receive_reply(can_socket, &can_frame, motorlimits);
    printf("motor exit\n");
